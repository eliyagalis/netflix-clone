...
import LandingForm from "../components/feature/LandingForm";
    ...
    return (
        <div className={`absolute left-0 bg-[rgb(15,15,15)] w-full min-h-screen`}>
            ...
            <div className="relative max-w-460 mx-auto">
                <Header>
                    <RoundedButton color="(rgb(255,255,255))" hover="(rgb(206,206,206))">Sign In</RoundedButton>
                </Header>
            </div>
            ...
            <div className="relative text-white z-50 w-8/10 max-w-350 mx-auto">
                <h1 className="text-2xl font-bold">{strings.landing.carousel.headline}</h1>
                <Carousel movies={movies} setFloatingPoster={(res: Movie) => setMovieDetails(res)} isIndexed />
                <br />
                ...
                <div className="py-5 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
                    {strings.landing.plans.cards.map((card, index) => (
                        <Card plans count={index} key={card.id} title={card.title} subtitle={card.subtitle} bulletPoints={card.bulletPoints} price={card.price}/>
                    ))}
                </div>
                <br />
